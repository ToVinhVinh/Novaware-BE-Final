...
class ProductViewSet(viewsets.ViewSet):
    ...
    def reviews(self, request, pk=None):
        ...
        Endpoint: POST /api/v1/products/{pk}/reviews
        Payload: {"rating": int, "comment": str}
        """
        ...
        # Use authenticated user's username or email as name
        user_name = request.user.username if hasattr(request.user, 'username') else str(request.user)
        ...
        if existing_review:
            ...
            existing_review.name = user_name
        ...
        review_serializer = ProductReviewSerializer(review)
        return api_success(
            "Review has been created" if created else "Review has been updated",
            {
                "review": review_serializer.data,
